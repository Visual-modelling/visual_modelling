...
class TransformerBlock(nn.Module):
  def __init__(self, d_model, heads, ff=True, residual=False, norm=False, masked=False):
    super().__init__()
    ...
    self.attention = SelfAttention(self.d_model, heads=heads, masked=masked)
...
class VMDecoder(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.tblocks = [TransformerBlock(d_model=self.in_dim, heads=self.heads, ff=self.ff,
        ...
        self.to_pixels = nn.Linear(self.in_dim, self.in_dim)

    def forward(self, x):
        # get input batch dimensions
        batch_dim, sequence_dim, height_dim, width_dim = x.shape

        # reshape image to vector
        x = x.view(batch_dim, sequence_dim, height_dim*width_dim)
        ...
        x = x.view(batch_dim, 1, height_dim, width_dim)
